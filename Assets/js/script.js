$(function () {
  ...
  saveButton.on("click", function () {
    //event listener for each button, function looks for buttons parents ID and for each it stores text area value
    localStorage.setItem($(this).parent().attr("id"), $(this).prev().val()); //
  });

  for (let i = 0; i < localStorage.length; i++) {
    //for each local storage
    ...
    $("#" + currentTask)
      .children(1)
      .val(localStorage.getItem(currentTask));
  }
